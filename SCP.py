...
def scp_iteration(f, s0, s_goal, s_prev, u_prev, N, P, Q, R, u_max, ρ):
    ...
    objective = 0.0
    constraints = []
    objective += cvx.quad_form(s_cvx[N] - s_goal, P) # Final state
    constraints.append(cvx.norm(s_cvx[N] - s_prev[N]) <= ρ) # Final state
    constraints.append(s_cvx[0] == s0)
    for k in range(N):
        objective += cvx.quad_form(s_cvx[k] - s_goal, Q) + cvx.quad_form(u_cvx[k], R) # Every kth state
        constraints.append(s_cvx[k + 1] == A[k] @ s_cvx[k] + B[k] @ u_cvx[k] + c[k]) # Dynamics
        constraints.append(cvx.norm_inf(s_cvx[k] - s_prev[k]) <= ρ) # Linearlization
        constraints.append(cvx.norm_inf(u_cvx[k] - u_prev[k]) <= ρ) 
        constraints.append(u_cvx[k] <= u_max) # u constraints
...
s_goal = np.array([1.5, 1.6, 0.5])  # desired upright pendulum state
s0 = np.array([0, 0, 0.5])  # initial downright pendulum state
dt = 0.1  # discrete time resolution
T = 10.0  # total simulation time
P = 1e3 * np.eye(n)  # terminal state cost matrix
Q = np.diag([1e-2, 1.0, 1e-3])  # state cost matrix
R = 1e-3 * np.eye(m)  # control cost matrix
ρ = 1.0  # trust region parameter
u_max = 2.0  # control effort bound
...
A = np.array([[6.01102422326145, 0,	0.561420513065361],
                [-1.01384238535083,	0, -0.0126146972409228],
                [0.507259693680044,	0, 1.20356857438518]])
B = np.array([[-2.20809753256568],
                  [0.423711851980508],
                  [-0.363821506814871]])
