...
AB = np.array(loadmat('Model_AB_new.mat')['AB'])
vine = np.array(loadmat('Model_AB_new.mat')['vine'])[0, 0]
...
# # # Load U from RL
U_all = loadmat('control_inputs.mat')
U_all = np.transpose(U_all['control_inputs'])
U_all = U_all.T
# print(U_all.shape)
...
x_drone = x[0,:]
plt.plot(time, x[0, :], label='x_drone')
print(x_drone[-1])
y_drone = x[1,:]
z_drone = x[2,:]
x_tip = x[-3,:]
y_tip = x[-2,:]
z_tip = x[-1,:]
plt.plot(time, x[-3,:], label='x_tip')
plt.plot(time, x[-2,:], label='y_tip')
plt.plot(time, x[-1,:], label='z_tip')
plt.legend()
