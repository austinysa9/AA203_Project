...
def animate_trajectories(time, x, y, z, x1, y1, z1, x2, y2, z2, trail_length_red=10, trail_length_blue=10):
    ...
    line1, = ax.plot([], [], [], 'r-', label='Drone (Red)', marker='o')
    line2, = ax.plot([], [], [], 'b-', label='Tip (Blue)', marker='o')
    line3, = ax.plot([], [], [], 'm-', label='Control Input (Magenta)', marker='o')

    # Initialize the dashed line connecting the newest points of the first two trajectories
    connector, = ax.plot([], [], [], 'g--', label='Vine (Green)')  # Green dashed line
